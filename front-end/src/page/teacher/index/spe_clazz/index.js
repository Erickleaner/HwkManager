...
import {clazzInsert, clazzList, clazzRemove, clazzUpdate} from "../../../../api/clazz";
...
const initConfirm = () => {
    $('#confirm').click(function() {
        ...
        if (operate===Operate.INSERT){
            row = emptyRow();
            bindEdit(row)
            clazzInsert(row).then(data=>{
                if (!data.isInsert){
                    row.clazzId = data.insertId
                    $('#myModal').modal('hide')
